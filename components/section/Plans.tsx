    ...
    return (
        <Section id="plans">
            <div className="mx-auto max-w-6xl px-6 flex items-center flex-col">
                ...
                <h1 className='text-left pb-[25px] md:text-[39px] text-[25px] dark:text-[#EBD9A1] text-[#293C4A]'>Our Plans</h1>
                            ...
                            <div key={index} className={`${checkLeftorRight(index + 1)}`}>
                                ...
                                <Card className="order-1 xl:w-5/12 p-6 flex flex-col flex-start gap-4">
                                    ...
                                    {plan.data.map((item, index) => (
                                        <p key={index} className="text-sm text-[#293C4A] dark:text-white">{item.description}</p>
                                    ))}
                                </Card>
                            </div>
                        ))}
